...
export default function CoachProfileScreen() {
  ...
  const fetchProfile = async (userId, token) => {
    try {
      setLoading(true);
      const res = await fetch(`http://10.0.2.2:5000/api/coach/user/${userId}`, {
  ...
  const handleCreateProfile = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch('http://10.0.2.2:5000/api/coach/', {
        method: 'POST',
        headers: {
  ...
  const handleEditProfile = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`http://10.0.2.2:5000/api/coach/${profileId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${user.token}`
  ...
  const handleDeleteProfile = async () => {
    ...
    Alert.alert('Xác nhận', 'Bạn có chắc muốn xóa hồ sơ?', [
      ...
      { text: 'Xóa', style: 'destructive', onPress: async () => {
        try {
          setLoading(true);
          const res = await fetch(`http://10.0.2.2:5000/api/coach/${profile._id}`, {
            method: 'DELETE',
