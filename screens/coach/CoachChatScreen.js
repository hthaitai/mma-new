...
export default function CoachChatScreen() {
  ...
  const fetchSessions = async (userId, token) => {
    try {
      ...
      setError(null);
      const res = await fetch(`http://192.168.2.4:5000/api/chat/sessions/${userId}`, {
  ...
  const handleCreateNewSession = async (userId, token) => {
    try {
      const res = await fetch(`http://192.168.2.4:5000/api/chat/`, {
        method: 'POST',
        headers: {
  ...
  const fetchChatHistory = async (chatId, token) => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch(`http://192.168.2.4:5000/api/chat/${chatId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
      if (res.ok) {
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch(`http://192.168.2.4:5000/api/chat/${chatId}/message`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
